...
# Load a 512x512 grayscale image (FIXED PATH)
image = cv2.imread(r'C:\Users\hp\Desktop\Image Processing Lab\ela.jpg', 0)

# Resize to 512x512
image = cv2.resize(image, (512, 512))

# Add Gaussian noise
noise = np.random.normal(0, 20, image.shape)  # mean=0, Standard deviation=20 This simulates sensor noise or distortion.
noisy_image = np.clip(image + noise, 0, 255).astype(np.uint8)#adds that noise to each pixel

# Show noisy image...unwanted random variations in brightness or color, which distort the original visual information
